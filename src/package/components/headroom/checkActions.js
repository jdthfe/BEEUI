function checkActions(states) {
    ...
    if (
        ...
        direction === 'down' && ['pinned', 'unfixed'].indexOf(states.state) >= 0 &&
        ...
        direction === 'up' &&
        distanceScrolled > states.upTolerance && ['pinned', 'unfixed'].indexOf(states.state) < 0
    ) {
        action = 'pin'
        ...
        direction === 'up' &&
        states.currentScrollY <= states.elemHeight && ['pinned', 'unfixed'].indexOf(states.state) < 0
    ) {
        action = 'pin'
    } else {
        action = 'none'
